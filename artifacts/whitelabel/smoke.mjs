...
try {
 for(const [app,port] of [['user',5290],['admin',5273]]) {
  ...
  let branding={displayName:'Camplight',logoUrl:'/brand/camplight-logo-white.svg',primaryColor:'#350950',accentColor:'#a65bd4',backgroundColor:'#f8f5ff'};
  ...
  await page.route('**/api/**', async route=>{
   ...
   if(url.pathname==='/api/branding'){if(route.request().method()==='PUT')branding=route.request().postDataJSON();data=branding;}
   else if(url.pathname==='/api/branding/access')data={canManage:true};
   else if(url.pathname==='/api/auth/me'){data=authenticated?{id:'user-1',username:'Alex',mustChangePassword:false}:{};status=authenticated?200:401;}
  ...
  await page.getByText('Sign in to Camplight', {exact:true}).waitFor();
  await page.evaluate(()=>document.fonts.ready);
  await page.screenshot({path:`artifacts/whitelabel/${app}-login.png`,fullPage:true,animations:"disabled"});
  ...
  await page.screenshot({path:`artifacts/whitelabel/${app}-desktop.png`,fullPage:true,animations:"disabled"});
  assert.equal(await page.title(), `Camplight | ${app==='admin'?'Admin':'Workspace'}`);
  assert.equal(await page.locator('.instance-sidebar-footer .powered-by-nest').count(),1);
  if(app==='admin') {
    await page.getByRole('button',{name:'Branding',exact:true}).click();
    await page.getByLabel('Organization name',{exact:true}).fill('Camplight Studio');
    await page.getByRole('button',{name:'Save branding',exact:true}).click();
    await page.getByRole('status').filter({hasText:'Branding saved'}).waitFor();
    await page.screenshot({path:'artifacts/whitelabel/branding-desktop.png',fullPage:true});
    await page.reload();
    await page.waitForFunction(() => document.title === 'Camplight Studio | Admin');
    await page.getByRole('button',{name:'Branding',exact:true}).click();
    assert.equal(await page.getByLabel('Organization name',{exact:true}).inputValue(),'Camplight Studio');
  }
  await page.getByRole('button',{name:'Switch to dark theme'}).click();
  assert.equal(await page.locator('html').getAttribute('data-theme'),'dark');
  await page.screenshot({path:`artifacts/whitelabel/${app}-dark.png`,fullPage:true,animations:"disabled"});
  await page.reload();
  ...
  if(app==='admin') {
   ...
   await page.getByRole('button',{name:'Branding',exact:true}).click();
   await page.getByRole('heading',{name:'Your brand. Your workspace.',exact:true}).waitFor();
  }
  await page.screenshot({path:`artifacts/whitelabel/${app}-mobile.png`,fullPage:true,animations:"disabled"});
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,`${app} mobile overflow`);
  if(app==='admin') {
   await page.screenshot({path:'artifacts/whitelabel/branding-mobile.png',fullPage:true});
   await page.getByRole('button',{name:'Menu',exact:true}).click();
   await page.waitForFunction(() => getComputedStyle(document.querySelector('.nest-sidebar')).transform === 'matrix(1, 0, 0, 1, 0, 0)');
   await page.screenshot({path:'artifacts/whitelabel/admin-mobile-sidebar.png',fullPage:true});

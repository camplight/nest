...
const TMP_ROOT = resolve(CLI_ROOT, ".release-tmp");
...
const DOCS_BUNDLE_NAME = "nest-system-docs.md";
const BUILD_INFO_NAME = "nest-build-info.json";
const SEA_CONFIG_NAME = "sea-config.json";
const SEA_BLOB_NAME = "nest-sea-prep.blob";
...
function outputNameForCurrentPlatform() {
  if (process.platform === "darwin") return "nest-macos";
  if (process.platform === "win32") return "nest-windows.exe";
  return "nest-linux";
}
...
async function buildReleaseExecutable() {
  ...
  const entryFile = resolve(CLI_ROOT, "src/index.ts");
  const bundledEntryFile = resolve(SNAPSHOT_DIR, "nest.cjs");
  await build({
    ...
    banner: {
      js: "// generated by nest build-release",
    },
  });

...
export function openUrlInBrowser(url: string) {
  if (isTruthyEnv((process.env.NEST_CLI_NO_BROWSER ?? process.env.ORGOPS_OPSCLI_NO_BROWSER))) {
    return;
  }
...
export function createUiShortcut(target: ShortcutTarget, url: string) {
  ...
  const title = target === "admin-ui" ? "Nest Admin UI" : "Nest User UI";
  ...
  const path = join(desktop, target === "admin-ui" ? "nest-admin-ui.desktop" : "nest-user-ui.desktop");
  const entry = [
    "[Desktop Entry]",
    "Type=Application",
    `Name=${title}`,

...
export function writeRunnerEnvConfig(installDir: string, config: RunnerBootstrapConfig) {
  ...
  const patch: Record<string, string> = {
    NEST_API_URL: config.apiUrl,
    NEST_RUNNER_TOKEN: config.runnerToken,
  };
  if (config.runnerName) patch.NEST_RUNNER_NAME = config.runnerName;
...
export function readRunnerEnvConfig(installDir: string): RunnerBootstrapConfig | null {
  ...
  const apiUrl = normalizeUrl(parsed.NEST_API_URL ?? parsed.ORGOPS_API_URL);
  const runnerToken = (parsed.NEST_RUNNER_TOKEN ?? parsed.ORGOPS_RUNNER_TOKEN ?? "").trim();
  const runnerName = (parsed.NEST_RUNNER_NAME ?? parsed.ORGOPS_RUNNER_NAME ?? "").trim();

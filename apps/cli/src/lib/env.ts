...
export function getNestCliEnvPath() {
  return join(process.cwd(), ROOT_ENV_FILE);
}

export function getModelId() {
  const configured = (process.env.NEST_CLI_MODEL ?? process.env.ORGOPS_OPSCLI_MODEL)?.trim();
...
async function askProviderChoice(ask: (prompt: string) => Promise<string>): Promise<ModelProvider> {
  while (true) {
    ...
    writeRoleMessage("nest", "Please choose 1, 2, or 3.");
  }
}
...
export async function ensureModelCredentials(input: {
  ...
  if (!hasAnyKey) {
    ...
    process.env.NEST_CLI_MODEL = modelId;
    writeMergedEnvFile(rootEnvPath, { NEST_CLI_MODEL: modelId });
    writeRoleMessage("nest", `Using model: ${modelId}`);
  }
  ...
  if (!process.env[requiredApiKeyEnv]?.trim()) {
    ...
    writeRoleMessage("nest", `Saved ${requiredApiKeyEnv} to ${rootEnvPath}`);
  }
  return true;
}

...
function macosLabelFor(component: InstallComponent) {
  return `com.nest.${componentSlug(component)}`;
}

function windowsTaskNameFor(component: InstallComponent) {
  if (component === "admin-ui") return "NestAdminUi";
  if (component === "user-ui") return "NestUserUi";
  if (component === "runner") return "NestRunner";
  return "NestApi";
}

function systemdServiceNameFor(component: InstallComponent) {
  return `nest-${componentSlug(component)}`;
}
...
export function registerAutostartServices(installDir: string, components: InstallComponent[]) {
  ...
  if (process.platform === "darwin") {
    ...
    for (const component of components) {
      ...
      const plist = [
        ...
        `<string>${escapeForDoubleQuotes(join(installDir, `.nest-${slug}.log`))}</string>`,
        "<key>StandardErrorPath</key>",
        `<string>${escapeForDoubleQuotes(join(installDir, `.nest-${slug}.err.log`))}</string>`,
        "</dict>",
        "</plist>",
        "",
      ].join("\n");
  ...
  if (process.platform === "win32") {
    ...
    for (const component of components) {
      ...
      const scriptPath = resolve(installDir, `.nest-start-${slug}.cmd`);
      const script = [
        "@echo off",
        `cd /d "${installDir}"`,
  ...
  for (const component of components) {
    ...
    const serviceBody = [
      "[Unit]",
      `Description=Nest component ${component}`,

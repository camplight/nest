...
const DEFAULT_REPO_URL = "https://github.com/camplight/nest.git";
const DEFAULT_REPO_REF = "main";
const INSTALL_SMOKE_MOCK_ENV = "NEST_CLI_INSTALL_SMOKE_MOCK";
...
function isInstallSmokeMockEnabled() {
  return isTruthyEnv((process.env[INSTALL_SMOKE_MOCK_ENV] ?? process.env.ORGOPS_OPSCLI_INSTALL_SMOKE_MOCK));
}
...
function ensureRepoReady(installDir: string, repoUrl: string, repoRef: string) {
  if (repoAlreadyPresent(installDir)) {
    runChecked("git", ["remote", "set-url", "origin", repoUrl], installDir);
...
function ensureRepoReadyMock(installDir: string, repoUrl: string, repoRef: string) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "nest-install-smoke", private: true }, null, 2)}\n`,
    "utf-8"
  );
  writeFileSync(
    resolve(installDir, ".nest-install-smoke.json"),
...
function markMockInstallBuild(installDir: string) {
  writeFileSync(resolve(installDir, ".nest-install-smoke-build.txt"), "mocked npm ci + npm run build\n", "utf-8");
}

function createMockShortcut(installDir: string) {
  const shortcutPath = resolve(installDir, "Nest User UI.smoke-shortcut");
...
export async function runInstall(rawOptions: InstallOptions) {
  ...
  const installDir = resolve(rawOptions.installDir ?? "nest");
  const repoUrl = rawOptions.repoUrl ?? process.env.NEST_REPO_URL ?? DEFAULT_REPO_URL;

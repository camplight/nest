...
function stateFile(installDir: string) {
  return resolve(installDir, ".nest-runtime", "admin-ui.json");
}

function resolveInstallDir(inputDir?: string) {
  return resolve(inputDir ?? loadState().installDir ?? "nest");
}
...
export async function startAndOpenAdminUi(options?: { installDir?: string; openBrowser?: boolean }) {
  ...
  if (!existing) {
    const runtimeDir = resolve(installDir, ".nest-runtime");
...
export function stopAdminUi(installDirFromArg?: string) {
  ...
  if (!existing) {
    return {
      installDir,
      stopped: false,
      message: "Admin UI is not running via nest runtime state.",
    };
  }
